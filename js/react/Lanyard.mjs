import React, { useEffect, useRef, useState } from 'https://esm.sh/react@17.0.2';
import { Canvas, extend, useFrame } from 'https://esm.sh/@react-three/fiber@8.13.1';
import { useGLTF, useTexture, Environment, Lightformer } from 'https://esm.sh/@react-three/drei@9.81.2';
import { BallCollider, CuboidCollider, Physics, RigidBody, useRopeJoint, useSphericalJoint } from 'https://esm.sh/@react-three/rapier@0.10.1';
import { MeshLineGeometry, MeshLineMaterial } from 'https://esm.sh/meshline@1.4.0';
...
export default function Lanyard({ position = [0, 0, 30], gravity = [0, -40, 0], fov = 20, transparent = true }) {
  return h('div', { className: 'lanyard-wrapper' },
    h(Canvas, {
      camera: { position, fov },
      ...
      h(Physics, { gravity, timeStep: 1 / 60 }, h(Band, null)),
...
function Band({ maxSpeed = 50, minSpeed = 0 }) {
  ...
  useFrame((state, delta) => {
    if (dragged) {
      ...
      card.current && card.current.setNextKinematicTranslation({ x: vec.x - dragged.x, y: vec.y - dragged.y, z: vec.z - dragged.z });
    }
    if (fixed.current) {
      [j1, j2].forEach((ref) => {
    ...
    h('group', { position: [0, 4, 0] },
      ...
      h(RigidBody, { position: [2, 0, 0], ref: card, ...segmentProps, type: dragged ? 'kinematicPosition' : 'dynamic' },
        ...
        h('group', {
          ...
          h('mesh', { geometry: nodes.card.geometry }, h('meshPhysicalMaterial', { map: materials.base.map, 'map-anisotropy': 16, clearcoat: 1, clearcoatRoughness: 0.15, roughness: 0.9, metalness: 0.8 })),
          h('mesh', { geometry: nodes.clip.geometry, material: materials.metal, 'material-roughness': 0.3 }),
